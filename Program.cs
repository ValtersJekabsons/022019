...
namespace ConsoleApp1
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string text = System.IO.File.ReadAllText(CSVFile);//read CSV file
            text = text.Replace(",,", ",");
            ...
            if (lines.Count <= 2) Environment.Exit(0); //if list has less than two element(cant create polygon) in it > then quit
            ...
            try
            {
                ...
                {
                    file.WriteLine("First line ID\t|Second Line ID\t|Location (x,y)");
                    for (int i = 0; i <= crossPoints.Count - 1; i++)
                    {
                        file.WriteLine(crossPoints[i].ID + "\t\t|" + crossPoints[i].ID2 + "\t\t|(" + crossPoints[i].COORD.X + " , " + crossPoints[i].COORD.Y + ")");
                    }
                }
            }
            catch (IOException e)
            {
                Console.WriteLine("Problem while outputting file");
            }
